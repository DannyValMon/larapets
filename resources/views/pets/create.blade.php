...
@section('content')
    ...
    <div class="card text-white md:w-[720px] w-[320px]">
        <form method="POST" action="{{ url('pets') }}" class="flex flex-col gap-4 mt-4" enctype="multipart/form-data">
            @csrf
            <div class="flex flex-col md:flex-row gap-4">
                ...
                <div class="w-full md:w-[320px]">
                    <div
                        ...
                        <div id="upload" class="mask mask-squircle w-48 bg-[#0006]">
                            <img id="preview" class="object-contain w-full h-full" src="" />
                        </div>
                ...
                <div class="w-full md:w-[320px]">
                    ...
                    <input type="text" class="input bg-[#0009] outline-0" name="name" placeholder="Bella"
                        value="{{ old('name') }}">
                    @error('name')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                    ...
                    <input type="number" class="input bg-[#0009] outline-0" name="weight" placeholder="12"
                        value="{{ old('weight') }}">
                    @error('weight')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                    ...
                    <input type="number" class="input bg-[#0009] outline-0" name="age" placeholder="3"
                        value="{{ old('age') }}">
                    @error('age')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                ...
                <div class="w-full md:w-[320px]">
                    ...
                    <input type="text" class="input bg-[#0009] outline-0" name="breed" placeholder="Labrador"
                        value="{{ old('breed') }}">
                    @error('breed')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                    ...
                    <input type="text" class="input bg-[#0009] outline-0" name="location" placeholder="Bogotá"
                        value="{{ old('location') }}">
                    @error('location')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                    ...
                    <input type="text" class="input w-full bg-[#0009] outline-0 text-white" name="description"
                        placeholder="Friendly and playful" value="{{ old('description') }}">
                    @error('description')
                    ...
                    <select class="select w-full bg-[#0009] outline-0 text-white" name="active"
                        ...
                        <option value="1" selected style="background-color: #1a1a2e; color: white;">Active</option>
                        <option value="0" style="background-color: #1a1a2e; color: white;">Inactive</option>
                    </select>
                    @error('active')
                        <small class="badge badge-error w-full mt-1 text-xs py-3">{{ $message }}</small>
                    @enderror
                    ...
                    <select class="select w-full bg-[#0009] outline-0 text-white" name="adopted"
                        style="background-color: rgba(0,0,0,0.6); color: white;">
                        <option value="0" selected style="background-color: #1a1a2e; color: white;">Not Adopted</option>
                        <option value="1" style="background-color: #1a1a2e; color: white;">Adopted</option>
                    </select>
                    @error('adopted')
                        <small class="badge badge-error w-full mt-1 text-xs py-3">{{ $message }}</small>
                    @enderror
            ...
            <div class="w-full flex justify-center mt-2">
                <button class="btn btn-outline hover:bg-[#fff6] hover:text-white w-1/2">Add</button>
            </div>
